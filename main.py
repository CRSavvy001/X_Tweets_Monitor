...
Commands:
  ...
  /debug <handle>                 show raw twitterapi.io response (diagnostic)
"""
...
def extract_handle(text: str) -> Optional[str]:
    ...
    Preserves the original casing — twitterapi.io's userName lookup is
    case-sensitive, so lowercasing here would break accounts that use
    mixed-case handles (e.g. CRSavvy001).
    """
    text = text.strip()
...
class BotApp:
    ...
    async def cmd_debug(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if not context.args:
            await update.message.reply_text("Usage: /debug <handle>")
            return
        handle = extract_handle(context.args[0])
        if not handle:
            await update.message.reply_text("Couldn't parse a handle from that.")
            return
        status, raw = await self.twitter.get_last_tweets_raw(handle)
        snippet = raw[:3500]
        await update.message.reply_text(f"HTTP {status}\n\n{snippet}")
...
def main():
    ...
    application.add_handler(CommandHandler("debug", bot_app.cmd_debug))
